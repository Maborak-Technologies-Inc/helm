...
*   **Recommendations**:
    ...
    *   **[Recommended] Example: IgnoreDifferences for HPA**
        ...
        ```
        *Why is this needed?*:
        Even though your chart templates have conditional logic (`if not .Values.backend.autoscaling.enabled`), **when HPA is ENABLED**:
        1. Helm renders `Rollout` with `replicas: X` (start state).
        2. HPA (Horizontal Pod Autoscaler) sees high load and scales it to `X + 5`.
        3. Argo CD checks Git (`replicas: X`) vs Control Plane (`replicas: X + 5`).
        4. Argo SD says "Drift Detected!" and tries to scale it back down to `X`.
        5. HPA scales it back up.
        This "fighting" is why `ignoreDifferences` is crucial *when* HPA is active.
...
*   **Recommendations**:
    *   **[Recommended] Example: Smart Database Probe (`pg_isready`)**
        The current `database-statefulset.yaml` correctly uses `pg_isready -U <user>`.
        *   *Check*: `readinessProbe`
        *   *Command*: `pg_isready -U {{ .Values.database.postgres.user }}`
        *   **Does this require a password?**:
            `pg_isready` is a utility that checks the connection status of a PostgreSQL server. It attempts to open a TCP connection and send a startup packet. If the server is accepting connections (even if it would eventually reject auth), it returns success (status 0).
            **It does NOT usually require a password** just to check availability.
            However, since it runs *inside* the container, it inherits the environment variables (including `POSTGRES_PASSWORD`), so if strict auth is enforced, it can silently use that environment variable to authenticate fully if needed.
            **This is the correct, production-grade way to probe Postgres.**
...
*   **Findings**:
    ...
    *   **Missing Prometheus Targets**:
        The chart has `Service` definitions (e.g., `backend-service.yaml`) but does *not* contain any `ServiceMonitor` or `PodMonitor` resources (Custom Resource Definitions used by Prometheus Operator).
        *   **"We don't have that?"**: Correct. I scanned the `templates/` directory, and while there are Istio Telemetry files (`backend-telemetry.yaml`), there are **no** `ServiceMonitor` files.
        *   **Impact**: If you install this in a cluster with Prometheus, Prometheus won't know to scrape metrics from your pods unless you manually add annotations or add a `ServiceMonitor`.

*   **Recommendations**:
    *   Add a `servicemonitor.yaml` template (wrapped in `{{ if .Values.serviceMonitor.enabled }}`) to allow automatic metric scraping.
